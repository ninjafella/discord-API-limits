class User:
    ...
    def __get_avatar(
        self,
    ):  # Changing the icon code of the user to the url that we then use on the webpage
        icon = self.avatar
        if (
            icon is not None
        ):  # If the icon id thingy exists create the link to it otherwise use the default discord icon
